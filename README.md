...
**1. Set the password.** The deploy dialog asks for **one** secret: `CDN_PASSWORD`, which gates
the explorer. It's a password field, so let your password manager generate it — it also signs your
session cookie unless you set `CDN_SESSION_SECRET`, which is what lets a one-click deploy ask you
for exactly one thing.

The other three are deliberately *not* in the dialog. `cdn setup` provisions two of them, and until
they exist the paths they guard are **closed**, not open: no `CDN_UPLOAD_TOKEN` means every bearer
is rejected (you can still drag files onto the explorer), and no `CDN_PURGE_TOKEN` means the purge
is local-POP only. Set any of them later with `wrangler secret put <NAME>`.

Auth fails **closed** throughout. With no password at all, `/login` says so and returns 503 rather
...
and the `description`s in `package.json` do not render for secrets. So a key in that file is a box
a stranger *must* type something into, with no explanation — and whatever they type becomes a real
credential. `packages/cli/src/deploy-dialog.test.ts` holds that shape in place.
...
| upload token | Generates `CDN_UPLOAD_TOKEN`, sets it on the Worker, and writes it to `~/.config/cdn/hosts/<domain>.env` — so the CLI, the hooks and the Shortcut work the moment setup finishes. **Never replaces an existing one.** |
...
**What it leaves you with.** A working custom domain, the cache policy set, and two credentials
that did not exist before — both *generated by setup rather than asked for*. `CDN_PURGE_TOKEN` is a
Cloudflare API token scoped to Cache Purge on one zone; `CDN_UPLOAD_TOKEN` is a bearer setup makes
itself, sets on the Worker, and writes to `~/.config/cdn/hosts/<domain>.env` at mode 0600, so
`cdn up` and the hooks work immediately with no second step. Neither is ever *replaced*: an
existing token belongs to writers on machines that are not this one, and overwriting it would break
...
- **`.dev.vars.example`** — **the Deploy to Cloudflare dialog, as a file.** One uncommented key
  (`CDN_PASSWORD`) with an empty value; the other three documented as comments. Every uncommented
  key becomes a required field in a stranger's deploy form and every value becomes its default, so
  editing this file is a product change, not a documentation one —
  `packages/cli/src/deploy-dialog.test.ts` says so out loud. Its second reader is local dev: copy
  it to `.dev.vars` and uncomment what you need.
...
Four, named in `.dev.vars.example`, and **one of them is required**. The Deploy to Cloudflare
button prompts for all four; otherwise they are one-time and shared by every version incl.
previews: `wrangler secret put CDN_PASSWORD` / `CDN_SESSION_SECRET` / `CDN_UPLOAD_TOKEN` /
`CDN_PURGE_TOKEN`. Setting them by hand needs a first deploy to exist. `CDN_UPLOAD_TOKEN` is a
bearer *you* issue, not a Cloudflare credential — file it in your password manager alongside app
tokens, not cloud ones.

| Secret | Prompted at deploy | Absent means |
| --- | --- | --- |
| `CDN_PASSWORD` | **yes, and required** | `/login` returns 503 saying so; no session can exist |
| `CDN_SESSION_SECRET` | no | the cookie key is derived from `CDN_PASSWORD` |
| `CDN_UPLOAD_TOKEN` | no | every bearer is rejected; no machine write path (`cdn setup` makes one) |
| `CDN_PURGE_TOKEN` | no | cache purge is local-POP only (`cdn setup` mints one) |

**If you deployed this template before 2026-09-07, rotate `CDN_SESSION_SECRET`.** Two defects, both
fixed: the deploy dialog prefilled `change-me` for every secret (from this repo's own
`.dev.vars.example`), and with *neither* `CDN_SESSION_SECRET` nor `CDN_PASSWORD` set the cookie key
collapsed to a constant published in this repo. Either way the session cookie was signed with a
value a stranger could read, so a forged cookie walked past the login page. Setting a real
`CDN_PASSWORD` (and `CDN_SESSION_SECRET`, if you use one) invalidates every cookie signed with the
old key.

...
**1. Set the password.** The deploy dialog asks for four secrets and **only the first is
required**: `CDN_PASSWORD` gates the explorer. It's a password field, so let your password manager
generate it — it also signs your session cookie unless you set `CDN_SESSION_SECRET`, which is what
lets a one-click deploy ask you for exactly one thing. Leave the other three blank; `cdn setup`
provisions them, and blank means the paths they guard are **closed**, not open: no
`CDN_UPLOAD_TOKEN` means every bearer is rejected (you can still drag files onto the explorer), and
no `CDN_PURGE_TOKEN` means the purge is local-POP only.
...
to check one against. `.dev.vars.example` documents all four and **ships them empty on purpose**:
the button prefills from that file's values, so a placeholder there is a default in everybody's
deploy dialog.
...
- **`.dev.vars.example`** — the four secrets in dotenv, **with empty values**. Two readers: the
  Deploy to Cloudflare button prompts from it — prefilling each field with the value written here,
  which is why they are empty — and you copy it to `.dev.vars` for local dev. Descriptions for the
  deploy dialog live in `package.json` under `cloudflare.bindings`; between them, those two files
  are the entire first-run UX, and they are worth editing as carefully as code.
...
| Secret | Required | Absent means |
| --- | --- | --- |
| `CDN_PASSWORD` | **yes** | `/login` returns 503 saying so; no session can exist |
| `CDN_SESSION_SECRET` | no | the cookie key is derived from `CDN_PASSWORD` |
| `CDN_UPLOAD_TOKEN` | no | every bearer is rejected; no machine write path |
| `CDN_PURGE_TOKEN` | no | cache purge is local-POP only |
